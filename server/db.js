...
const queries = {
  ...
  insertBet: async (bet) => {
    ...
    if (bet.telegram_msg_id) {
      await dbRun('INSERT OR IGNORE INTO processed_messages (msg_id, is_bet) VALUES (?, 1)', [bet.telegram_msg_id]);
    }
    return res.lastID;
  },

  markMessageProcessed: async (msgId, isBet = 0) => {
    if (!msgId) return;
    await dbRun('INSERT OR IGNORE INTO processed_messages (msg_id, is_bet) VALUES (?, ?)', [msgId, isBet ? 1 : 0]);
  },

  getProcessedMessageIds: async () => {
    const rows = await dbAll('SELECT msg_id FROM processed_messages');
