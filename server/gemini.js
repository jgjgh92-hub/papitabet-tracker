...
// Cargar lista de API keys (soporta una sola key o varias separadas por coma en GEMINI_API_KEY / GEMINI_API_KEYS)
function getApiKeys() {
...
function getActiveClient() {
  ...
  // Si todas están marcadas como agotadas, verificar si podemos reintentar
  console.log('🔄 [Gemini] Todas las API keys estaban marcadas como agotadas. Reseteando estado para reintentar...');
  exhaustedKeys.clear();
...
function rotateToNextKey(exhaustedKey) {
  if (exhaustedKey) {
    ...
    console.warn(`⚠️ [Gemini] Key #${currentKeyIndex + 1} (${exhaustedKey.substring(0, 8)}...) agotada. Rotando a la siguiente...`);
  }
...
async function getWorkingModel(genAI) {
  ...
  return { model: genAI.getGenerativeModel({ model: 'gemini-2.0-flash' }), name: 'gemini-2.0-flash' };
}
 ...
 * Soporta rotación automática multi-key: si una key da 429, rota a la siguiente key disponible.
 */
async function analyzeBetImage(imagePath, keyAttempt = 0) {
  ...
  if (keyAttempt >= keys.length && keys.length > 0 && exhaustedKeys.size >= keys.length) {
    console.warn('⛔ [Gemini] Todas las API keys disponibles han alcanzado su cuota diaria. Se reintentará en el próximo ciclo.');
    return {
      success: false,
      is_valid_bet: false,
  ...
  try {
    ...
    if (err.message.includes('429')) {
      ...
      // Si solo hay 1 key y es rate limit por minuto
